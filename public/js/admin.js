  ...
  const html = `
    ...
    <body>
      ...
      <div style="text-align:center; margin-bottom:20px;">
        <span class="badge ${type === 'summary' ? 'badge-cyan' : 'badge-gold'}">${type.toUpperCase()}</span>
        <p>Quiz ID: ${currentReportData.quizId}</p>
        <p>Generated on: ${new Date(currentReportData.timestamp).toLocaleString()}</p>
      </div>
      <hr>
      ${content}
      <footer style="margin-top:50px; text-align:center; color:#999; font-size:12px;">
        Generated by Perfect Quiz System
      </footer>
    </body>
    </html>
  `;
